  ...
  async getBookingsByEmail(req, res) {
    try {
      ...
      if (bookings.length === 0) {
        return res.status(404).json({
          data: null,
          message: `No bookings found for email: ${email}`,
          status: STATUS.NOT_FOUND,
        });
      }
      return res.status(200).json({
...
router.get(
  '/api/v1/bookings/by-user/:email',
  bookingController.getBookingsByEmail.bind(bookingController),
);
router.get(
