...
describe("BookingService Unit Tests", () => {
  ...
  describe("getBookingsByUser", () => {
    it("should find bookings by user email/password and select out password", async () => {
      const mockBooking = { userEmail: "test@email.com", password: "pass", toObject: () => ({ userEmail: "test@email.com", password: "pass" }) };
      ...
      expect(result).toEqual([{ userEmail: "test@email.com" }]);
    });
  });
